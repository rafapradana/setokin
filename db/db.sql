...
CREATE OR REPLACE FUNCTION generate_batch_number(p_item_id UUID)
RETURNS VARCHAR AS $$
DECLARE
    v_count INTEGER;
...
BEGIN
    ...
    v_batch_number := 'BATCH-' || TO_CHAR(CURRENT_DATE, 'YYYYMMDD') || '-' || LPAD((v_count + 1)::TEXT, 4, '0');
...
CREATE TRIGGER trigger_check_batch_depletion BEFORE UPDATE ON batches
    FOR EACH ROW EXECUTE FUNCTION check_batch_depletion();
